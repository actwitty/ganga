...
describe "access control" do
  ...
  before(:each) do
    ...
    resp = page.driver.post('/app/create', { format: 'json', account_id: @account._id, description: { :email => "john.doe@example.com", domain: "http://rulebot.com"}})
  ...
  it "should not authenticate api" do
    # lets make out of session request
    # wrong token
    post('/app/update',{ format: 'json', token: "534645645645", id: @json["id"], description: { :email => "tom.doe@example.com", mobile: "943479474"}},  { "HTTP_HOST" => "" })
    puts JSON.parse(response.body)
    response.status.should_not eq(200)
  end
  ...
  it "should authenticate api" do
    ...
    #request object is not available in request spec
    post('/app/update', { format: 'json', token: @json["description"]["token"], id: @json["id"], description: { :email => "tom.doe@example.com", mobile: "943479474"}},  { "HTTP_HOST" => "" })
    puts JSON.parse(response.body)
    response.status.should eq(200)
  end
end